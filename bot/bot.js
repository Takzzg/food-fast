...
const fetchProducts = async()=>{
    ...
    const response = await axios.get(`${api}/products?filterOrder=price&sortOrder=-1`)
    const products = response.data.map(e=>{return {
        id: e._id,
        nombre: e.name,
        precio: e.price,
    ...
    bot.telegram.sendMessage(ctx.chat.id, startMessage,{
        reply_markup: {
            inline_keyboard: [
                ...
                [
                    {text: "Nuestra website", url: "https://food-fast-client-8h45out5u-takzzg.vercel.app/"}
                ]
            ],
        }
...
bot.action('0', async(ctx) =>{
    ...
    reply_markup: {
        inline_keyboard: [
            ...
            [
                 { text: "Pagar",  url: "https://food-fast-client-8h45out5u-takzzg.vercel.app/" }
            ],
            [
...
bot.action('Salir', ctx => {
    ctx.deleteMessage()
    const message =  "Gracias por tu visita :) , si queres comprar podes presionar /start"
    bot.telegram.sendMessage(ctx.chat.id, message, {
        reply_markup: {
