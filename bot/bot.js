...
bot.on('text', async(ctx)=>{
    const response = await axios.get(`${api}/products?name=${ctx.update.message.text}`)
    
    if(response.data.error) return  ctx.reply("Ups!!! no encontramos tu producto 😭")
...
ctx.deleteMessage()
await bot.telegram.sendPhoto(ctx.chat.id, response.data[0].image.secure_url)
bot.telegram.sendMessage(ctx.chat.id, message, {
        reply_markup: {
            inline_keyboard: [
                [{ text: "+info", url: url }],
